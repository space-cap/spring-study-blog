...
@Repository("userDaoImpl") // @Repository는 @Component의 특수화된 형태
...
//        PreparedStatement pstmt = null;
		...
		try (Connection conn = ds.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql); // SQL
			...
			pstmt.setDate(4, new java.sql.Date(user.getBirth().getTime())); // 디비의 date타입은 java.sql.Date로 처리
			pstmt.setString(5, user.getSns());
			pstmt.setTimestamp(6, new java.sql.Timestamp(user.getReg_date().getTime())); // 디비의 datetime타입은
																						// java.sql.Timestamp로 처리
